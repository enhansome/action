...
- [ ] **Empty-tree parses — parser yield fixes** · CRITICAL → `progress/empty-tree-parses.md` — 8 diagnosed root causes on a measured step plan (yield harness + goldens; steps 0–6 landed 2026-08-24, yield 77.8%→97.7% offline, `java` 0→783/785, `crypto` 0→995/999); next: step 7 (own-link in later paragraphs). The webapp hollow-registry stock rides this thread.

...
- [ ] **repo_info data quality** · HIGH → `progress/repo-info-quality.md` — wrong fields, stale/suspect flags, NULLs that hide repos from filters, rename currency; diagnose the failure class from mirror JSON first.
- [ ] **Emit `first_seen` per item** · HIGH → `progress/first-seen.md` — the mirror's history is the only source of listing-addition dates; without them "what's new" is unanswerable downstream.
